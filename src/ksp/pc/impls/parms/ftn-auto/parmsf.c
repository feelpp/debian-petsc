...
extern "C" {
#endif
void PETSC_STDCALL  pcparmssetglobal_(PC pc,PCPARMSGlobalType *type, int *__ierr ){
*__ierr = PCPARMSSetGlobal(
	(PC)PetscToPointer((pc) ),*type);
}
void PETSC_STDCALL  pcparmssetlocal_(PC pc,PCPARMSLocalType *type, int *__ierr ){
*__ierr = PCPARMSSetLocal(
	(PC)PetscToPointer((pc) ),*type);
}
void PETSC_STDCALL  pcparmssetsolvetolerances_(PC pc,PetscReal *tol,PetscInt *maxits, int *__ierr ){
*__ierr = PCPARMSSetSolveTolerances(
	(PC)PetscToPointer((pc) ),*tol,*maxits);
}
void PETSC_STDCALL  pcparmssetsolverestart_(PC pc,PetscInt *restart, int *__ierr ){
*__ierr = PCPARMSSetSolveRestart(
	(PC)PetscToPointer((pc) ),*restart);
}
void PETSC_STDCALL  pcparmssetnonsymperm_(PC pc,PetscBool *nonsym, int *__ierr ){
*__ierr = PCPARMSSetNonsymPerm(
	(PC)PetscToPointer((pc) ),*nonsym);
}
void PETSC_STDCALL  pcparmssetfill_(PC pc,PetscInt *lfil0,PetscInt *lfil1,PetscInt *lfil2, int *__ierr ){
