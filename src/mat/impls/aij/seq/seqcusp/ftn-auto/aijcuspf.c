...
#undef __FUNCT__
#define __FUNCT__ ""
void PETSC_STDCALL  matcuspsetformat_(Mat A,MatCUSPFormatOperation *op,MatCUSPStorageFormat *format, int *__ierr ){
*__ierr = MatCUSPSetFormat(
	(Mat)PetscToPointer((A) ),*op,*format);
}
void PETSC_STDCALL  matcreateseqaijcusp_(MPI_Fint * comm,PetscInt *m,PetscInt *n,PetscInt *nz, PetscInt nnz[],Mat *A, int *__ierr ){
